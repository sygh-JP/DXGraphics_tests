================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : MfcD3D10Test1 プロジェクトの概要
===============================================================================

この MfcD3D10Test1 アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、MfcD3D10Test1 アプリケーションを構成する各ファイルの内容の概要が含まれています。

MfcD3D10Test1.vcxproj
...
MfcD3D10Test1.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CMfcD3D10Test1App アプリケーション クラスを宣言します。

MfcD3D10Test1.cpp
    これはメインのアプリケーション ソース ファイルで、CMfcD3D10Test1App アプリケーション クラスが含まれています。
...
res\MfcD3D10Test1.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル MfcD3D10Test1.rc にインクルードされます。

res\MfcD3D10Test1.rc2
...
MfcD3D10Test1Dlg.h, MfcD3D10Test1Dlg.cpp - ダイアログ
    これらのファイルには、CMfcD3D10Test1Dlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる MfcD3D10Test1.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、MfcD3D10Test1.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

MfcD3D10Test1.manifest
