================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : MfcD2DEffectTest1 プロジェクトの概要
===============================================================================

この MfcD2DEffectTest1 アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、MfcD2DEffectTest1 アプリケーションを構成する各ファイルの内容の概要が含まれています。

MfcD2DEffectTest1.vcxproj
...
MfcD2DEffectTest1.vcxproj.filters
...
MfcD2DEffectTest1.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CMfcD2DEffectTest1App アプリケーション クラスを宣言します。

MfcD2DEffectTest1.cpp
    これはメインのアプリケーション ソース ファイルで、CMfcD2DEffectTest1App アプリケーション クラスが含まれています。

MfcD2DEffectTest1.rc
...
res\MfcD2DEffectTest1.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル MfcD2DEffectTest1.rc にインクルードされます。

res\MfcD2DEffectTest1.rc2
...
MfcD2DEffectTest1Dlg.h, MfcD2DEffectTest1Dlg.cpp - ダイアログ
    これらのファイルには、CMfcD2DEffectTest1Dlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる MfcD2DEffectTest1.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、MfcD2DEffectTest1.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

MfcD2DEffectTest1.manifest
